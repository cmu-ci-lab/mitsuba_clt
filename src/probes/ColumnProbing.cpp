...
class ColumnProbing : public BasicProbe{
public:
    typedef TSpectrum<half, SPECTRUM_SAMPLES> SpectrumHalf;
    typedef TMIPMap<Spectrum, SpectrumHalf> MIPMap;
    ColumnProbing(const Properties &props) : BasicProbe(props) {
        m_rowTransform = 0;
        m_type = "column";
    }

    ColumnProbing(Stream *stream, InstanceManager *manager)
    ...
    Spectrum sampleProjectorDirection(DirectionSamplingRecord &dRec,
        ...
        if (extra) {
            Point2 resamplePosition(extra->x,
                                    (int)std::ceil(m_projectorResolution.y * sampler->next1D()));
        ...
        dRec.pdf = (Float)m_projectorResolution.x * m_projectorNormalization / (d.z * d.z * d.z);

        return Spectrum(m_scale/(Float)m_projectorResolution.x);
    }
    ...
    Float pdfProjectorDirection(const DirectionSamplingRecord &dRec,
        ...
        if(((xPos - pRec.cameraUV.x - m_colTransform)<1.f &&
            ...
            return importance(d, EPROJ) * (Float) m_projectorResolution.x;
        }
        return  0.0f;
    }
    ...
    Spectrum sampleProjectorDirect(DirectSamplingRecord &dRec, const Point2 &sample) const {
        ...
        int32_t targetPos = math::floorToInt(dRec.cameraUV.x + m_colTransform) % m_projectorSize.x;
...
MTS_IMPLEMENT_CLASS_S(ColumnProbing, false, BasicProbe)
MTS_EXPORT_PLUGIN(ColumnProbing, "Column Probing Pattern");
