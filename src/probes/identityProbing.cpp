...
class IdentityProbing : public BasicProbe{
public:
    typedef TSpectrum<half, SPECTRUM_SAMPLES> SpectrumHalf;
    typedef TMIPMap<Spectrum, SpectrumHalf> MIPMap;
    IdentityProbing(const Properties &props) : BasicProbe(props) {
        m_type = "identity";
    }

    IdentityProbing(Stream *stream, InstanceManager *manager)
    ...
    Spectrum sampleProjectorDirection(DirectionSamplingRecord &dRec,
        ...
        if (extra) {
            ...
            samplePos.x = (extra->x + sample.x) * m_projectorInvResolution.x;
            samplePos.y = (extra->y + sample.y) * m_projectorInvResolution.y;
        }
        ...
        dRec.pdf = (Float)m_projectorResolution.x * m_projectorResolution.y * m_projectorNormalization
                    / (d.z * d.z * d.z);

        return Spectrum(m_scale/((Float)m_projectorResolution.x * m_projectorResolution.y));
    }
    ...
    Float pdfProjectorDirection(const DirectionSamplingRecord &dRec,
        ...
        Point sample = m_projectorToSample(samplePoint * m_projectorNearClip);
        Float xPos = sample.x * m_projectorSize.x, yPos = sample.y * m_projectorSize.y;

        if(((xPos - pRec.cameraUV.x - m_colTransform)<1.f &&
            (xPos - pRec.cameraUV.x - m_colTransform)>=0.f) &&
            ((yPos - pRec.cameraUV.y - m_rowTransform)<1.f &&
            (yPos - pRec.cameraUV.y - m_rowTransform)>=0.f)){
            return importance(d, EPROJ) * (Float) m_projectorResolution.x * m_projectorResolution.y;
        }
        return  0.0f;
    }
    ...
    Float pdfCameraDirection(const DirectionSamplingRecord &dRec,
        ...
        if((xPos - floor(pRec.cameraUV.x) > 1.f || xPos < pRec.cameraUV.x) ||
           (yPos - floor(pRec.cameraUV.y) > 1.f || yPos < pRec.cameraUV.y)) {
            return 0.f;
        }
    ...
    Spectrum evalProjectorDirection(const DirectionSamplingRecord &dRec,
        ...
        && ((yPos - pRec.cameraUV.y - m_rowTransform)<1.f && (yPos - pRec.cameraUV.y - m_rowTransform)>=0.f)){
    ...
    Spectrum evalCameraDirection(const DirectionSamplingRecord &dRec,
        ...
        Point samplePoint(p.x,p.y,1.0f);
        Point sample = m_cameraToSample(samplePoint * m_cameraNearClip);
        Float xPos = sample.x * m_cameraResolution.x, yPos = sample.y * m_cameraResolution.y;

        if(xPos - floor(pRec.cameraUV.x) > 1.f || xPos < floor(pRec.cameraUV.x)||
           yPos - floor(pRec.cameraUV.y) > 1.f || yPos < floor(pRec.cameraUV.y)) {
            return Spectrum(0.f);
        }
    ...
    Spectrum sampleProjectorDirect(DirectSamplingRecord &dRec, const Point2 &sample) const {
        ...
        int32_t targetPosX = math::floorToInt(dRec.cameraUV.x + m_colTransform) % m_projectorSize.x;
        int32_t targetPosY = math::floorToInt(dRec.cameraUV.y + m_rowTransform) % m_projectorSize.y;
        if (dRec.uv.x < targetPosX || dRec.uv.x -  (Float)targetPosX > 1.0f ||
            dRec.uv.y < targetPosY || dRec.uv.y -  (Float)targetPosY > 1.0f ) {
            dRec.pdf = 0;
            return Spectrum(0.0f);
        }
    ...
    Spectrum sampleCameraDirect(DirectSamplingRecord &dRec, const Point2 &sample) const {
        ...
        if (dRec.uv.x - floor(dRec.cameraUV.x) > 1.f ||dRec.uv.x < floor(dRec.cameraUV.x) ||
            dRec.uv.y - floor(dRec.cameraUV.y) > 1.f ||dRec.uv.y < floor(dRec.cameraUV.y)){
            dRec.pdf = 0;
            return Spectrum(0.0f);
        }
...
MTS_IMPLEMENT_CLASS_S(IdentityProbing, false, BasicProbe)
MTS_EXPORT_PLUGIN(IdentityProbing, "identity Probing Pattern");
