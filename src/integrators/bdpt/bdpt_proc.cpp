...
public:
	...
	void process(const WorkUnit *workUnit, WorkResult *workResult, const bool &stop) {
		...
		for (size_t i=0; i<m_hilbertCurve.getPointCount(); ++i) {
			...
			for (size_t j = 0; j<m_sampler->getSampleCount(); j++) {
				...
				if(m_scene->getProbe()!=NULL){
					Path::alternatingRandomWalkforProbe(m_scene, m_sampler,
    ...
    void evaluate(BDPTWorkResult *wr,
        ...
        bool isProbe = probe != NULL && probe->getType().compare("disparity") == 0;
        Spectrum sampleValue(0.0f);
        if (isProbe) {
            PathVertex *vsPred, *vtPred = sensorSubpath.vertexOrNull(0),
                    *vs, *vt = sensorSubpath.vertex(1),
                    *vScene = sensorSubpath.vertex(2);
            PathEdge *vsEdge;
            vScene->sampleDirect(scene, m_sampler,
                                 &tempEndpoint, &tempEdge, &tempSample, EImportance);
            vs = &tempSample;
            vsPred = &tempEndpoint;
            vsEdge = &tempEdge;
            PositionSamplingRecord posR1 = vt->getPositionSamplingRecord(),
                    posR2 = vs->getPositionSamplingRecord();
            float rDisparity = posR2.uv.x - posR1.uv.x;
            float gDisparity = posR2.uv.y - posR1.uv.y;
            float disparities[3] = {rDisparity, gDisparity, 0.f};
            sampleValue = Spectrum(disparities);
        } else {
           ...
           while respecting the specified maximum path length */
                ...
                for (int t = maxT; t >= minT; --t) {
                    ...
                    if (vs->isEmitterSupernode()) {
                        ...
                        /* Caroline comment: cast checks if the camera is a surface camera
                         * that can interact with */
                        if (!vt->cast(scene, PathVertex::EEmitterSample) || vt->isDegenerate()) {
                            continue;
                        }

                        //TODO: Add getSamplePosition for Emitter so that
                        //		it is symmetrical with sensor

                        value = radianceWeights[t] *
                                vs->eval(scene, vsPred, vt, EImportance) *
                                vt->eval(scene, vtPred, vs, ERadiance);
                    } else if (vt->isSensorSupernode()) {
                        /* Caroline comment: cast checks if the emitter is a surface light source
                           * that can interact with */
                        ...
                        if (s == 1) {
                            if (vt->isDegenerate())
                                continue;
                            PositionSamplingRecord emitterPRec = emitterSubpath.vertex(1)->getPositionSamplingRecord();
                            /* Generate a position on an emitter using direct sampling */
                            value = radianceWeights[t] * vt->sampleDirect(scene, m_sampler,
                                                                          &tempEndpoint, &tempEdge, &tempSample,
                                                                          EImportance,
                                                                          emitterPRec.uv, emitterPRec.cameraUV);
                            ...
                            if (vs->isDegenerate())
                                continue;
                            PositionSamplingRecord sensorPRec = sensorSubpath.vertex(1)->getPositionSamplingRecord();
                            //TODO: Add probability adjustment for probing
                            /* Generate a position on the sensor using direct sampling */
//                          if(m_config.lightImage && t==1) {
//                          emitterSubpath.vertex(1)->pdf[EImportance] = emitterSubpath.vertex(1)->pdf[EImportance] / 512.f;
//                          emitterSubpath.vertex(1)->weight[EImportance]   = emitterSubpath.vertex(1)->weight[EImportance] * 512.f;
//                          }

                            value = importanceWeights[s] * vs->sampleDirect(scene, m_sampler,
                                                                            &tempEndpoint, &tempEdge, &tempSample,
                                                                            ERadiance,
                                                                            sensorPRec.uv, sensorPRec.cameraUV);
                            /* TODO: Add conditional probability adjustment for probing
                                If we want the thing to be corrected for MIWeight */
//							if (m_config.lightImage && probe == Probe::ECOLUMN) {
//								emitterSubpath.vertex(1)->pdf[EImportance] =
//										emitterSubpath.vertex(1)->pdf[EImportance] / 512.f;
//								emitterSubpath.vertex(1)->weight[EImportance] =
//										emitterSubpath.vertex(1)->weight[EImportance] * 512.f;
//								value *= 512.f;
//							}
                            if (value.isZero()) {
                                continue;
                        ...
                        if (vs->isDegenerate() || vt->isDegenerate())
                            continue;
                        Spectrum val1 = vs->eval(scene, vsPred, vt, EImportance);
                        Spectrum val2 = vt->eval(scene, vtPred, vs, ERadiance);
             ...
             function that are coupled to the connection edge */
               ...
               modify the path to reflect this change */
                    ...
                    /* TODO: Add conditional probability adjustment for probing
                    	If we want the thing to be corrected for MIWeight */
//					if (probe == Probe::ECOLUMN && m_config.lightImage && sampleDirect && t == 1 && s > 1) {
//						emitterSubpath.vertex(1)->pdf[EImportance] = emitterSubpath.vertex(1)->pdf[EImportance] * 512.f;
//						emitterSubpath.vertex(1)->weight[EImportance] =
//								emitterSubpath.vertex(1)->weight[EImportance] / 512.f;
//						value /= 512.f;
//					}
        ...
        wr->putSample(initialSamplePos, sampleValue, isProbe);
    }
