 ...
 */
    ...
    public:
        ...
        PerspectiveEmitterWhite(const Properties &props) : PerspectiveEmitter(props) {
            ...
            m_rowTransform = props.getInteger("rowTransform", 1.0f);
            m_colTransform = props.getInteger("colTransform", 1.0f);
        ...
        inline Float importance(const Vector &d) const {
            ...
            */
            ...
            return m_normalization * invCosTheta
                   * invCosTheta * invCosTheta * m_resolution.x * m_resolution.y;
        }
...
//            return Spectrum(m_scale);
            return importance(v) *  Spectrum(m_scale)/((Float)m_resolution.x * (Float)m_resolution.y);
                   ///((Float)m_resolution.x * (Float)m_resolution.y);
        }
...
//            std::cout<<screenSample.toString()<<endl;
            ...
            return Spectrum(importance(localD) * invDist * invDist * m_scale)/((Float)m_resolution.x * (Float)m_resolution.y);
            //
                   //((Float)m_resolution.x * (Float)m_resolution.y);
